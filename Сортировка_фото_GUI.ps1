...
# Функция для определения скриншотов по имени файла
...
# Функция для проверки сжатых медиафайлов (<100KB)
function Test-IsCompressedMedia {
    param([string]$FilePath)
    
    $fileItem = Get-Item $FilePath
    
    # Файлы меньше 100KB считаем сжатыми медиа
    if ($fileItem.Length -lt 100KB) {
        Write-Host "    Маленький файл (<100KB), перемещаем в Compressed_Media" -ForegroundColor Blue
        return $true
    }
...
$CompressedMediaPath = Join-Path $DestinationRoot "Compressed_Media"
...
Write-Host "  Папка сжатых медиа: $CompressedMediaPath" -ForegroundColor Blue
...
Write-Host "Файлы меньше 100KB будут перемещены в папку Compressed_Media." -ForegroundColor Blue
...
$null = New-Item -ItemType Directory -Path $CompressedMediaPath -Force
...
for ($i = 0; $i -lt $totalFiles; $i++) {
    ...
    if ($i % $BatchSize -eq 0 -or $i -eq $totalFiles - 1) {
        ...
        Write-Progress -Activity "Обработка файлов" -Status "$i/$totalFiles ($percent%) | Скриншоты: $screenshotCount | Сжатые: $compressedMediaCount | Другие: $otherFilesCount | EXIF: $exifCount | Имена: $nameCount | Система: $systemCount" -PercentComplete $percent
    }
    
    try {
        # 1. Проверяем поддерживаемый формат файла
        ...
        # 2. Проверяем, является ли файл скриншотом (ВЫСШИЙ ПРИОРИТЕТ)
        ...
        # 3. Проверяем, является ли файл сжатым медиа (<100KB)
        if (Test-IsCompressedMedia -FilePath $file.FullName) {
            Write-Host "  Сжатое медиа обнаружено: $($file.Name)" -ForegroundColor Blue
            
            # Проверка дубликатов для сжатых медиа
            if (Test-FileDuplicate -SourceFile $file.FullName -TargetFolder $CompressedMediaPath) {
                $duplicateDestination = Join-Path $DoublePath $file.Name
                $counter = 1
                while (Test-Path $duplicateDestination) {
                    $baseName = [System.IO.Path]::GetFileNameWithoutExtension($file.Name)
                ...
                $destinationFile = Join-Path $CompressedMediaPath $file.Name
                $counter = 1
                while (Test-Path $destinationFile) {
                    $baseName = [System.IO.Path]::GetFileNameWithoutExtension($file.Name)
                    $extension = [System.IO.Path]::GetExtension($file.Name)
                    $destinationFile = Join-Path $CompressedMediaPath "$baseName-$counter$extension"
                    $counter++
                }
        ...
        # 4. Обычная обработка для поддерживаемых файлов (не скриншотов и не сжатых)
...
Write-Host "  Сжатые медиа (<100KB): $compressedMediaCount файлов" -ForegroundColor Blue
...
Write-Host "  Сжатые медиа перемещены: $compressedMediaCount" -ForegroundColor Blue
